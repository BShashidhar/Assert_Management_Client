...
export class ManagementFileUploadComponent implements OnInit {

  uploader: FileUploader = new FileUploader({ url: Api.BASE_URL + Api.uploadFile });
  response: any
  filename: any
  fileNameList: any = []
  nameList: any = []
  ...
  grcirForm: FormGroup
  grcirFDateValue : Date = null;
  poFDateValue : Date = null;
  billFDateValue : Date = null;
  res : number;
  a: any;
  ...
  constructor(
    ...
    private _MrrpoService : MrrpoService
  ) {
  ...
  public get f() { return this.grcirForm.controls; }

  addGrcirF() {
    this.submitted = true
    this.saved = true
    if (this.grcirForm.invalid) return
    let grcirForm = {
